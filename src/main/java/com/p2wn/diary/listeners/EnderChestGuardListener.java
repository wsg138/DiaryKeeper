...
public final class EnderChestGuardListener implements Listener {

    private final DiaryPlugin plugin;

    public EnderChestGuardListener(DiaryPlugin plugin) {
        this.plugin = plugin;
    }

    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    public void onInventoryClick(InventoryClickEvent event) {
        if (!plugin.restrictionService().isEnderChestRestrictionEnabled()
                || !plugin.restrictionService().isEnderChestTop(event.getView())) {
            return;
        }
